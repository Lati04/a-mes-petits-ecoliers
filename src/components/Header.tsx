...
const Header: React.FC<HeaderProps> = ({ navItems = defaultNavItems }) => {
  ...
  return (
    <header className="w-full px-4 py-2 sticky top-0 z-50 header-rainbow backdrop-blur-md shadow-[0_6px_15px_rgba(224,175,71,0.25)] dark:bg-[#3d2e00]/90 dark:shadow-[0_6px_15px_rgba(224,175,71,0.4)] transition-all duration-300">
     ...
     <CTAButton
      ...
      >
        <>
          ...
          />
          <span className="text-base sm:text-xl">À mes petits écoliers</span>
        </>
      </CTAButton>
      ...
      {/* ---------- MENU MOBILE ---------- */}
      {menuOpen && (
        <ul className="mt-2 list-none transition-all duration-300 origin-top transform shadow-lg md:hidden header-rainbow backdrop-blur-md">
          {navItems.map((item) => (
